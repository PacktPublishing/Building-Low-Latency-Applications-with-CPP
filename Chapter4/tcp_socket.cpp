...
auto TCPSocket::connect(const std::string &ip, const std::string &iface, int port, bool is_listening) noexcept -> int {
  destroy();
  fd_ = createSocket(logger_, ip, iface, port, false, false, is_listening, 0);
  return fd_;
}
...
auto TCPSocket::sendAndRecv() noexcept -> bool {
  const auto n_rcv = recv(fd_, rcv_buffer_ + next_rcv_valid_index_, TCPBufferSize - next_rcv_valid_index_, MSG_DONTWAIT);
  if (n_rcv > 0) {
